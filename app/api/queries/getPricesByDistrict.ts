...
export async function getPricesByDistrict(
  ...
  if (calculation_type === 'monthly') {
    return prisma.$queryRaw`
      ...
      JOIN agmarknet.master_district md ON p.district_id = md.district_id
      JOIN agmarknet.master_commodities mc ON p.commodity_id = mc.commodity_id
      WHERE p.commodity_id = ${commodity_id}
        AND p.district_id = ${district_id}
        AND p.date BETWEEN ${start_date} AND ${end_date}
      GROUP BY month, tsd.district_id, md.district_name, mc.commodity_name
      ORDER BY month DESC;
    `;
  } else {
    return prisma.$queryRaw`
      ...
      WHERE p.commodity_id = ${commodity_id}
        AND p.district_id = ${district_id}
        AND p.date BETWEEN ${start_date} AND ${end_date}
      GROUP BY p.date, tsd.district_id, md.district_name
