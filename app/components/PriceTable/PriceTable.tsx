...
export const PriceTable: React.FC<PriceTableProps> = ({
  ...
  const calculatePriceChange = (dataArray: PriceData[]) => {
    return dataArray.map((data, index, array) => {
      const currentPrice = parseFloat(data.avg_modal_price);  // Ensure this is a number
      const previousPrice = index < array.length - 1 ? parseFloat(array[index + 1].avg_modal_price) : 0;
  ...
  const calculateQtyChange = (dataArray: QuantityData[]) => {
    return dataArray.map((data, index, array) => {
      const currentQty = parseFloat(data.total_quantity);  // Ensure this is a number
      const previousQty = index < array.length - 1 ? parseFloat(array[index + 1].total_quantity) : 0;
      const qtyChange = previousQty - currentQty;
      const changeFormatted =
        previousQty === 0
          ? "N/A"
